...
This script takes the original dataset and produces a single processed dataset ready for ML.
"""
...
def main():
    ...
    print("=== FLIGHT DATA PROCESSING PIPELINE ===")

    # Load original data
    data_path = Path("dataset/flights_sample_3m.csv")
    ...
    # Step 2: Handle missing values and create targets
    df = handle_missing_values_and_create_target(df)

    # Step 3: Engineer temporal features
    df = engineer_temporal_features(df)

    # Step 4: Engineer route features
    df = engineer_route_features(df)

    # Step 5: Preprocess for ML
    target_column = "is_delayed_15min"
    X_train, X_test, y_train, y_test, preprocessor, label_encoders = preprocess_for_ml(df, target_column)

    # Step 6: Save processed data
    feature_names = save_processed_data(X_train, X_test, y_train, y_test, preprocessor, target_column)
